...
void drawJapanMap() {
  // HOKKAIDO - High detail northern island
  const float hokkaido[][2] = {
    {45.52, 141.35}, {45.48, 142.15}, {45.38, 142.95}, {45.18, 143.65},
  ...
  // HONSHU - Main island with detailed coastline
  const float honshu[][2] = {
    // North tip
    {41.48, 141.08}, {41.18, 140.58}, {40.78, 140.28}, {40.38, 139.98},
    ...
    {35.58, 136.78}, {35.38, 136.98}, {35.28, 137.38}, {35.18, 137.88},
    // Tokyo Bay area (bump east)
    {35.28, 138.48}, {35.28, 138.98}, {35.18, 139.48}, {35.08, 139.78},
    {34.98, 139.88}, {34.88, 139.78}, {34.78, 139.58}, {34.68, 139.28},
    // South coast
    {34.58, 138.88}, {34.38, 138.38}, {34.18, 137.78}, {33.98, 137.18},
    {33.88, 136.48}, {33.88, 135.78}, {33.98, 135.18}, {34.08, 134.58},
    // Inland Sea area
    {34.28, 134.08}, {34.58, 133.58}, {34.98, 133.18}, {35.38, 132.88},
    {35.88, 132.68}, {36.38, 132.88}, {36.88, 133.28}, {37.38, 133.78},
    // Back up north on west coast
    {37.88, 134.38}, {38.38, 135.08}, {38.88, 135.88}, {39.38, 136.78},
    {39.88, 137.78}, {40.38, 138.88}, {40.88, 139.88}, {41.28, 140.68},
    {41.48, 141.08}
  };
  
  // KYUSHU - Southern island
  const float kyushu[][2] = {
    {33.88, 131.18}, {33.68, 131.58}, {33.48, 131.88}, {33.18, 132.08},
  ...
  // SHIKOKU - Fourth main island (between Honshu and Kyushu)
  const float shikoku[][2] = {
    {34.08, 134.58}, {33.88, 134.78}, {33.58, 134.68}, {33.38, 134.38},
    {33.28, 133.98}, {33.28, 133.48}, {33.48, 133.08}, {33.78, 132.78},
    {34.08, 132.68}, {34.38, 132.88}, {34.58, 133.28}, {34.58, 133.78},
    {34.38, 134.18}, {34.08, 134.58}
  };
...
void drawCaliforniaMap() {
  ...
  // San Andreas Fault (detailed)
  const float fault[][2] = {
    {40.00, -124.00}, {38.50, -122.80}, {37.80, -122.20},
    {37.00, -121.80}, {36.00, -120.50}, {35.20, -119.80},
    {34.50, -118.90}, {34.00, -118.35}, {33.50, -117.50}, {33.00, -116.50}
  };
...
void checkForEarthquakes() {
  ...
  // Always process the first earthquake on initial boot
  bool isNewQuake = (quakeID != lastQuakeID && quakeID.length() > 0 && lastQuakeID.length() > 0);
  bool needsInitialData = (lastQuakeID.length() == 0);  // First fetch ever
  
  if (isNewQuake || needsInitialData) {
    // Always update lastQuakeID if it's a new quake (but not on initial load)
    if (isNewQuake) {
      lastQuakeID = quakeID;
    }
    ...
    if (isInRegion(lat, lon, config.region)) {
      ...
      if (isNewQuake && mag >= config.magThreshold) {
        Serial.println("ALERT! Magnitude above threshold!");
        displayEarthquakeAlert(&latestQuake);
      }
      
      // Set lastQuakeID after first successful load
      if (needsInitialData) {
        lastQuakeID = quakeID;
        Serial.println("Initial data loaded, quakeID saved");
      }
      
      lastActivity = millis();
    } else {
      Serial.println("Latest quake outside region bounds");
    }
  } else {
    Serial.println("No new earthquake (same ID as last check)");
  }
}
