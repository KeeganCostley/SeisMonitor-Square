...
void drawJapanMap() {
  ...
  for (int i = 0; i < points - 1; i++) {
    int x1 = mapLonToScreen(japan[i][1]);
    int y1 = mapLatToScreen(japan[i][0]);
    int x2 = mapLonToScreen(japan[i+1][1]);
    int y2 = mapLatToScreen(japan[i+1][0]);
    if (x1 >= MAP_X && x1 <= MAP_X + MAP_WIDTH) {
      tft.drawLine(x1, y1, x2, y2, currentTheme.mapOutline);
    }
  }
  
  // Cities
  int tokyoX = mapLonToScreen(139.69);
  int tokyoY = mapLatToScreen(35.68);
  if (tokyoX >= MAP_X && tokyoX <= MAP_X + MAP_WIDTH) {
    tft.fillCircle(tokyoX, tokyoY, 1, currentTheme.mapCity);
  }
  
  int osakaX = mapLonToScreen(135.50);
  int osakaY = mapLatToScreen(34.69);
  if (osakaX >= MAP_X && osakaX <= MAP_X + MAP_WIDTH) {
    tft.fillCircle(osakaX, osakaY, 1, currentTheme.mapCity);
  }
}
...
void drawTaiwanMap() {
  ...
  // Cities
  tft.fillCircle(mapLonToScreen(121.56), mapLatToScreen(25.03), 1, currentTheme.mapCity); // Taipei
  tft.fillCircle(mapLonToScreen(120.31), mapLatToScreen(22.63), 1, currentTheme.mapCity); // Kaohsiung
}
...
void drawCaliforniaMap() {
  ...
  // Cities
  tft.fillCircle(mapLonToScreen(-122.42), mapLatToScreen(37.77), 1, currentTheme.mapCity); // SF
  tft.fillCircle(mapLonToScreen(-118.24), mapLatToScreen(34.05), 1, currentTheme.mapCity); // LA
  tft.fillCircle(mapLonToScreen(-117.16), mapLatToScreen(32.72), 1, currentTheme.mapCity); // SD
}
...
void drawGlobalMap() {
  ...
  for (int y = centerY - radius; y <= centerY + radius; y += 2) {
    int dx = sqrt(radius * radius - (y - centerY) * (y - centerY));
    tft.drawPixel(centerX - dx/2, y, currentTheme.seismoGrid);
    tft.drawPixel(centerX + dx/2, y, currentTheme.seismoGrid);
  }
  
  // Pacific Ring of Fire
  for (int angle = -30; angle <= 210; angle += 2) {
    float rad = angle * 0.0174533;
    int x = centerX + (radius - 5) * cos(rad);
    int y = centerY + (radius - 5) * sin(rad);
...
void checkForEarthquakes() {
  ...
  if (highestIdx >= 0) {
    ...
    highestRegionalQuake.latitude = usingNZ ? h["geometry"]["coordinates"][1] : h["geometry"]["coordinates"][1];
    highestRegionalQuake.longitude = usingNZ ? h["geometry"]["coordinates"][0] : h["geometry"]["coordinates"][0];
    highestRegionalQuake.depth = usingNZ ? h["properties"]["depth"] : h["geometry"]["coordinates"][2];
    
    const char* loc = usingNZ ? h["properties"]["locality"] : h["properties"]["place"];
    strncpy(highestRegionalQuake.location, loc, sizeof(highestRegionalQuake.location) - 1);
  ...
  String quakeID = usingNZ ? latest["properties"]["publicID"].as<String>() : latest["id"].as<String>();
  
  if (quakeID != lastQuakeID && quakeID.length() > 0) {
    lastQuakeID = quakeID;
    ...
    if (isInRegion(lat, lon, config.region)) {
      ...
      Serial.println("NEW QUAKE: M" + String(mag));
      
      if (mag >= config.magThreshold) {
