...
void updateLatestActivity() {
  ...
  if (lastQuake.hasData) {
    ...
    // Show time ago below in medium text
    tft.setTextSize(2);
    tft.setCursor(5, 180);
    ...
    // Show location across multiple lines
    tft.setTextSize(1);
    String loc = lastQuake.locality;
    
    // Split location into multiple lines (18 chars per line)
    int startPos = 0;
    int lineY = 205;
    while (startPos < loc.length() && lineY < 235) {
      String line;
