...
void drawSeismographScreen() {
  ...
  uint16_t neonCyan = 0x07FF;      // Bright cyan #00FFFF
  uint16_t neonMagenta = 0xF81F;   // Bright magenta #FF00FF  
  uint16_t electricBlue = 0x039F;  // Electric blue
  uint16_t dimCyan = 0x0410;       // Dim cyan for accents
  uint16_t darkGrey = 0x2945;      // Dark grey for grid
  
  // === ANGULAR BORDER FRAMES ===
  tft.drawRect(0, 0, 160, 240, electricBlue);
  tft.drawRect(1, 1, 158, 238, electricBlue);
  tft.drawRect(161, 0, 159, 240, neonMagenta);
  tft.drawRect(162, 1, 157, 238, neonMagenta);
  
  // === CORNER ACCENTS (angular cuts) ===
  tft.fillTriangle(0, 0, 10, 0, 0, 10, neonCyan);
  tft.fillTriangle(320, 0, 310, 0, 320, 10, neonCyan);
  tft.fillTriangle(0, 240, 10, 240, 0, 230, neonCyan);
  tft.fillTriangle(320, 240, 310, 240, 320, 230, neonCyan);
  
  // === VERTICAL DIVIDER with GLOW ===
  tft.drawLine(160, 0, 160, 240, neonCyan);
  tft.drawLine(159, 0, 159, 240, dimCyan);
  tft.drawLine(161, 0, 161, 240, dimCyan);
  
  // === LEFT SIDE - SEISMOGRAPH ===
  tft.setTextSize(1);
  tft.setTextColor(neonCyan);
  tft.setCursor(8, 8);
  tft.print("// SEISMO");
  ...
  for (int y = 40; y < 195; y += 20) {
    for (int x = 5; x < 155; x += 4) {
      tft.drawPixel(x, y, darkGrey);
    }
  }
  
  // Draw centerline with GLOW effect
  tft.drawLine(5, SEISMO_CENTER-1, 155, SEISMO_CENTER-1, dimCyan);
  tft.drawLine(5, SEISMO_CENTER, 155, SEISMO_CENTER, TFT_WHITE);
  tft.drawLine(5, SEISMO_CENTER+1, 155, SEISMO_CENTER+1, dimCyan);
  
  // === LATEST ACTIVITY BOX (angular corners) ===
  tft.drawRect(3, 125, 154, 112, neonMagenta);
  // Corner cut effect
  tft.drawLine(3, 125, 10, 125, TFT_BLACK);
  tft.drawLine(3, 125, 3, 132, TFT_BLACK);
  tft.fillTriangle(3, 125, 10, 125, 3, 132, neonCyan);
  
  // === RIGHT SIDE - MAP ===
  tft.setTextColor(neonMagenta);
...
void updateLatestActivity() {
  ...
  // Cyberpunk colors
  uint16_t neonCyan = 0x07FF;
  uint16_t neonMagenta = 0xF81F;
  ...
  if (displayQuake->hasData) {
    // Magnitude in BIG neon text (scaled by fontSize)
    ...
    // Time ago in dim cyan
    tft.setTextSize(1);
    tft.setTextColor(0x0410);  // Dim cyan
    tft.setCursor(8, 220);
...
void animateSeismograph() {
  ...
  // Cyberpunk colors
  uint16_t neonGreen = 0x07E0;    // Bright green
  uint16_t dimGreen = 0x0300;     // Dim green for trail
  uint16_t dimCyan = 0x0410;
  ...
  tft.drawLine(seismographX, lastY, seismographX + 1, newY, neonGreen);
  
  // Add glow trail (draw dimmer line behind)
  if (seismographX > 6) {
    tft.drawLine(seismographX-1, lastY, seismographX, newY, dimGreen);
  }
  ...
  if (eraseX % 4 == 0) {
    tft.drawPixel(eraseX, SEISMO_CENTER, 0x2945);
  }
...
void drawDetailedNZMap() {
  ...
  for (int i = 0; i < northPoints - 1; i++) {
    ...
    tft.drawLine(x1, y1, x2, y2, mapColor);
  }
...
// ========== JAPAN MAP ==========
void drawJapanMap() {
  uint16_t mapColor = 0x5AEB;
  uint16_t cityColor = 0x4E9F;
  ...
  if (tokyoX >= MAP_X && tokyoX <= MAP_X + MAP_WIDTH) {
    tft.fillCircle(tokyoX, tokyoY, 2, cityColor);
  }
  ...
  if (osakaX >= MAP_X && osakaX <= MAP_X + MAP_WIDTH) {
    tft.fillCircle(osakaX, osakaY, 2, cityColor);
  }
}
...
void drawTaiwanMap() {
  ...
  tft.fillCircle(taipeiX, taipeiY, 2, cityColor);
  
  // Kaohsiung
  int kaohsiungX = mapLon(120.31);
  int kaohsiungY = mapLat(22.63);
  tft.fillCircle(kaohsiungX, kaohsiungY, 2, cityColor);
}
...
void drawCaliforniaMap() {
  ...
  tft.fillCircle(sfX, sfY, 2, cityColor);
  
  // Los Angeles
  int laX = mapLon(-118.24);
  int laY = mapLat(34.05);
  tft.fillCircle(laX, laY, 2, cityColor);
  
  // San Diego
  int sdX = mapLon(-117.16);
  int sdY = mapLat(32.72);
  tft.fillCircle(sdX, sdY, 2, cityColor);
}

// ========== GLOBAL MAP (Stylized Globe) ==========
void drawGlobalMap() {
  uint16_t mapColor = 0x5AEB;
  uint16_t gridColor = 0x2945;
  ...
  tft.drawCircle(centerX, centerY, radius, mapColor);
  tft.drawCircle(centerX, centerY, radius-1, mapColor);
  ...
  for (int lat = -60; lat <= 60; lat += 30) {
    ...
    for (int x = centerX - halfWidth; x <= centerX + halfWidth; x += 2) {
      ...
      if (abs(y - yTop) < 2) tft.drawPixel(x, yTop, gridColor);
      if (abs(y - yBottom) < 2) tft.drawPixel(x, yBottom, gridColor);
    }
  }
  ...
  for (int lon = -150; lon <= 150; lon += 60) {
    ...
    for (int y = centerY - radius; y <= centerY + radius; y += 2) {
      ...
      if (abs(x - xLeft) < 2) tft.drawPixel(xLeft, y, gridColor);
      if (abs(x - xRight) < 2) tft.drawPixel(xRight, y, gridColor);
    }
  }
