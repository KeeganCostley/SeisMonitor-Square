    ...
    if (httpCode > 0) {
      ...
      if (features.size() > 0) {
        ...
        if (highestIndex >= 0) {
          ...
          if (isGlobalMode) {
            if (!highestGlobal.hasData || highestMag > highestGlobal.magnitude) {
              highestGlobal.magnitude = highestQuake["properties"]["mag"];
              ...
              highestGlobal.depth = highestQuake["geometry"]["coordinates"][2];
              highestGlobal.time = "global";
              highestGlobal.timestamp = millis();
              highestGlobal.hasData = true;
            }
          } else {
            if (!highestNZ.hasData || highestMag > highestNZ.magnitude) {
              highestNZ.magnitude = highestQuake["properties"]["magnitude"];
...
void fadeToSeismograph() {
  // Cool retro color fade effect!
  // Fade through colors: Red -> Orange -> Yellow -> Green -> Cyan -> Blue -> Purple -> Black
  uint16_t fadeColors[] = {
    TFT_RED,
    TFT_ORANGE, 
    TFT_YELLOW,
