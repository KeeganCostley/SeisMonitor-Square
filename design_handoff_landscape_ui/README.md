...
- `06-alert-routine.png` — Screen 5, Alert · M2.4 routine
- `07-alert-notable.png` — Screen 5, Alert · M5.5 notable
- `08-alert-severe.png` — Screen 5, Alert · M7.1 severe
- `09-alert-longname.png` — Screen 5, Alert · M3.9 long place name

---

## Screen 5 — Alert ("SEISMIC ACTIVITY DETECTED")

**File:** `screens/sm-land-alert.jsx` (`Alert04a`). Design host: `../SeisMonitor Alert Options.html`.

**Purpose:** Full-bleed takeover when a new quake meets the user's sensitivity threshold (defaults M2.0, so this fires several times a day and is mostly small quakes). Holds **25s** (`ALERT_DURATION`), then returns to the monitor. It is a "something just happened" notice, **not** an emergency siren — severity is carried by **colour**, not by shouting. Header copy is fixed: **`SEISMIC ACTIVITY DETECTED`**.

**Composition — diagonal corner burst (deliberately unlike the 3-panel monitor):**
- Background flat `bg` `#03080a`. No panels, no map, no seismograph — **one** quake only.
- **Activity rings** emanate from the **bottom-left corner**, origin at **(16, 224)**. This off-centre origin is the whole point — the shockwave sweeps up and across the screen, so the composition is diagonal/asymmetric rather than centred.
- Content is anchored to the **right**, clear of the ring origin:
  - **Header** top-right, right-aligned, two lines: `SEISMIC ACTIVITY` / `DETECTED`. **Inter/FreeSansBold, 15px**, weight 800, line-height 1.15, letter-spacing 0.5, coloured by severity. (Two lines is what lets it be large — the single-line string is 352px at 12pt and does not fit 320px; see brief §5.2.)
  - **Magnitude** `M7.1` bottom-right, right-aligned, **Inter 60px, weight 600** (light, not chunky — this was tuned down from an initial heavy 900/96px), letter-spacing −1.5, severity-coloured. Baseline ~54px from the bottom.
  - **Place name** right-aligned, **Inter 22px weight 700**, colour `ink` `#b8e6c4`. ~30px from bottom. Long names (e.g. "20km SW of Tokomaru Bay") stay on one line at 22px and fit; do **not** shrink to fit — keep the size and let it run close to the left edge.
  - **"NOW · {depth}KM DEEP"** bottom-right, **13px** mono, severity-coloured. This is the "showing now" framing (no timestamp).

**Rings — geometry & animation:**
- 5 rings, each a circle stroked in the severity colour, **2px** wide, expanding from the origin.
- Each animates scale **0.06 → 1.0** with opacity **1 → 0** over **2.4s**, `cubic-bezier(0,.45,.5,1)`, staggered by `dur/count` (≈0.48s) so a new ring launches as the previous fades — a repeating shockwave. Max radius reaches **~360px** (rings sweep the full screen and off the top-right; that's intended).
- A small **origin core** dot (6px, severity colour) marks the epicenter at (16,224).
- **Entrance beat:** the prototype uses the "hard" ring easing (bright, fast out) which doubles as the arrival hit; on device, fire one ring hard on arrival, then settle into the loop.

**Device implementation notes (brief §5.3–5.4):**
- Rings crossing the whole screen **is the design** — build it with **`drawSmoothArc`** (anti-aliased, variable-thickness, over the flat black background) rather than 1px `drawCircle`. Because rings pass under/over text, use the **re-stamp** or **draw-order** escape hatch (§5.4): redraw the header/magnitude/place text on top each frame after the rings repaint. Text is cheap; target **~18fps / 55ms** as the current alert already achieves.
- No blur/glow on device — the severity colour on flat black already reads as phosphor. If a glow is wanted, use the house trick: a second, dimmer 1px arc offset outward.
- **Fonts to generate** (brief §5.2 — you can rasterise any size): magnitude ~60px, header ~15px (two lines), place ~22px, meta ~13px. All ASCII; place names are pre-romanised.

**Severity ramp — PROPOSED (we agree the firmware's is backwards).** The current ramp isn't monotonic in luminance: M7 `#a54142` is the *dimmest* pixel on a black screen, so the worst quake is the quietest. Replace with a ramp that climbs in **both luminance and saturation** (`sevColor()` in the file):

| Magnitude | Hex | Reads as |
|---|---|---|
| < 4.0 | `#7fd69a` | calm phosphor green (routine — 90% of alerts) |
| ≥ 4.0 | `#cfe25f` | chartreuse |
| ≥ 5.0 | `#ffcf47` | amber |
| ≥ 6.0 | `#ff9538` | orange |
| ≥ 7.0 | `#ff5d4d` | hot red — brightest & most saturated |

On an additive display brightness = urgency, so worse is now unmistakably hotter and brighter. These values are used only here and for map markers, per the brief.

**Answers to the brief's §10 questions:**
1. *Distinct without being a different product* — same palette, same phosphor rings, same Inter/mono type; distinctness comes from **structure** (off-centre diagonal burst vs. the calm 3-panel instrument), not new visual language.
2. *Composition* — anchored bottom-left origin + right-aligned content; deliberately not centred.
3. *What the rings mean* — here they're the shockwave (hero). Encoding magnitude in ring count/reach is prototyped in the options host (Option 5) if you want them to carry data later.
4. *Severity ramp* — replaced (above).
5. *"NOW" framing* — kept literal `NOW · {depth}KM DEEP`; it tested clearest and reuses existing glyphs.

**States rendered** (`screenshots/06–09`): M2.4 routine (green), M5.5 notable (amber), M7.1 severe (red), M3.9 long-name stress test.
...
- `screens/sm-land-alert.jsx` — Screen 5 (Alert · `Alert04a`; other explorations kept in the same file for reference).
